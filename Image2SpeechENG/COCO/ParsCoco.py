...
# Set the paths
annotation_file = 'C:\\Users\\karen\\PhD\\SpeechSynthesisPhD\\Image2SpeechENG\\COCO\\cocotext.v2.json'
image_folder = 'C:\\Users\\karen\\PhD\\SpeechSynthesisPhD\\Image2SpeechENG\\COCO\\images'

# Initialize the COCO-Text API
data = COCO_Text(annotation_file)

# Load an image and its annotations
img_id = data.getImgIds()[0]  # Get the first image ID
img_info = data.loadImgs(img_id)[0]  # Load image information
ann_ids = data.getAnnIds(imgIds=img_id)  # Get annotation IDs for this image
anns = data.loadAnns(ann_ids)  # Load the annotations
