...
class AudioPlayer:
    ...
    def open_config_file(self):
        config = filedialog.askopenfilenames(initialdir="/", title="Select config file",
    ...
    def __init__(self):
        ...
        self.out_csv_file_path = out_metadata_file_path
        ...
        text = TextUtilities.read_docx_file(in_text_file_path)
        ...
        self.scan_directory()
    def __init__(self, in_text_file_path, waves_dir_path, out_metadata_file_path):
        self.logger = logging.getLogger("error_logger")
        self.logger.setLevel(logging.ERROR)
        file_handler = logging.FileHandler("Error.log")
        formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
        ...
        if not os.path.exists(in_text_file_path):
            self.logger.error(f"{in_text_file_path} File not exist:")
            return
        if not os.path.exists(out_metadata_file_path):
            self.logger.error(f"{out_metadata_file_path} File not exist:")
            return
        if not os.path.exists(waves_dir_path):
            self.logger.error(f"{waves_dir_path} Directory not exist:")
            return
    ...
    def scan_directory(self):
        ...
        files = sorted(files, key=lambda x: os.stat(os.path.join(self.dir_path, x)).st_ctime)
        for file in files:
            if file.endswith(".mp3") or file.endswith(".wav"):
                filename = os.path.join(self.dir_path, file)
    ...
    def save_pair(self):
        ...
        if self.played_song_index is not None:
            ...
            print(f"text: {text}")
            with open(self.out_csv_file_path, "a", newline="",
...
player = AudioPlayer(in_text_file_path=Config.adamamutin_docx_file_path,
                     waves_dir_path=Config.adamamutin_output_dir_path,
                     out_metadata_file_path=Config.adamamutin_player_output_csv_file_path)
player.run()
